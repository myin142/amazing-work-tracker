...
const overrideSystemHandling = (e: Event) => {
  if (e) {
...
const checkHeldKeysRecursive = (
  ...
  if (shortcutIndexOfKey === 0) {
    ...
    if (shortcutKeyRecursionIndex != null && shortcutKeyRecursionIndex > 0)
...
const useKeyboardShortcut = (
  ...
  const heldKeys = useRef([] as string[]);
  ...
  const keydownListener = useCallback(
    (keydownEvent: KeyboardEvent) => {
      ...
      if (
        ...
        BLACKLISTED_DOM_TARGETS.indexOf(
          (keydownEvent.target as Element).tagName
        ) >= 0
      ) {
        return;
      }
